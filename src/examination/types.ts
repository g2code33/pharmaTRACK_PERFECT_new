...
export type CrossDeviceRecoveryState =
  | 'NOT_STARTED'
  | 'ACTIVE'
  | 'DEVICE_LOST'
  | 'RECOVERY_PENDING'
...
export type ResultVisibilityPolicy = 'IMMEDIATE' | 'ON_RELEASE' | 'NEVER';

export type ResultReleaseStatus = 'HIDDEN' | 'RELEASED' | 'PENDING_SYNC' | 'FAILED';

export interface ExamResultDetailSettings {
  showScore: boolean;
...
export const DEFAULT_RESULT_DETAILS: ExamResultDetailSettings = {
  showScore: true,
  showPercentage: true,
  showPassFail: true,
...
export interface ExamResultReleaseRecord {
  id: string;
  sessionId: string;
  examId: string;
  ...
  attemptId: string;
  studentId: string;
  status: ResultReleaseStatus;
  releasedAt?: string;
  releasedByAdminId?: string;
  releasedByDeviceSessionId?: string;
  revokedAt?: string;
...
export interface ResultReleaseAuditRecord {
  id: string;
  examId: string;
  examVersionId: string;
  attemptId: string;
  studentId: string;
  previousState: ResultReleaseStatus;
  newState: ResultReleaseStatus;
  adminId: string;
...
export interface StudentVisibleResult {
  visible: boolean;
  status: ResultVisibilityPolicy | ResultReleaseStatus;
  message: string;
  attemptId: string;
  examId: string;
  examTitle: string;
...
export interface ExamVersion {
  ...
  resultDetails?: ExamResultDetailSettings;
}
...
export interface StudentAttempt {
  ...
  recoveryStatus?: CrossDeviceRecoveryState;
}
...
export interface SyncEvent {
  id: string;
  sessionId: string;
  entity: 'SESSION' | 'ATTEMPT' | 'ANSWER' | 'SECURITY_EVENT' | 'ADMIN_ACTION' | 'RECOVERY' | 'SUBMISSION';
  entityId: string;
...
export interface ExamReplicationPayload {
  ...
  resultReleases?: ExamResultReleaseRecord[];
  resultReleaseAudits?: ResultReleaseAuditRecord[];
}
