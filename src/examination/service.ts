...
import {
  ...
  type SecurityEvent,
  type StudentAttempt,
  type CrossDeviceRecoveryState,
  type ResultVisibilityPolicy,
  type ResultReleaseStatus,
  type ExamResultDetailSettings,
  type ExamResultReleaseRecord,
  type ResultReleaseAuditRecord,
...
export interface CreateVersionInput {
  ...
  resultDetails?: Partial<ExamResultDetailSettings>;
}
...
export class ExaminationRepository {
  ...
  private pendingSyncItems: Array<{
    sessionId: string;
    events: SyncEvent[];
  ...
  private syncBatchRunning = false;
  ...
  async save(): Promise<boolean> {
    ...
    this.savePromise = Promise.resolve().then(async () => {
      try {
        return await saveExaminationState(this.state);
      } finally {
  ...
  getCrossDeviceRecoveryState(
    sessionId: string,
    studentId: string,
  ): CrossDeviceRecoveryState {
    const attempt = this.state.attempts.find(
      (item) => item.sessionId === sessionId && item.studentId === studentId,
    );
    if (!attempt) return 'NOT_STARTED';
    if (attempt.recoveryStatus) return attempt.recoveryStatus;
    ...
    if (attempt.status === 'DEVICE_LOST') return 'DEVICE_LOST';
    if (attempt.status === 'RECOVERY_PENDING') return 'RECOVERY_PENDING';
  ...
  async createVersion(
    ...
    const versionWithoutHash: Omit<ExamVersion, 'versionHash'> = {
      ...
      resultVisibilityPolicy: input.resultVisibilityPolicy || 'IMMEDIATE',
      resultDetails: {
        ...DEFAULT_RESULT_DETAILS,
        ...input.resultDetails,
      },
  ...
  async closeSession(sessionId: string): Promise<ExamSession> {
  ...
  async createAttempt(
    ...
    if (existing) {
      ...
      const newDev = this.state.deviceSessions.find((item) => item.id === deviceSessionId);
      if (newDev?.platform) {
        existing.platform = newDev.platform;
      }
      existing.lastSyncedAt = new Date().toISOString();
      existing.ownershipGeneration = (existing.ownershipGeneration || 0) + 1;
      existing.status = 'ACTIVE';
      existing.recoveryStatus = 'RECOVERED';
    ...
    const attempt: StudentAttempt = {
      ...
      settingsSnapshot: {
        ...
        resultVisibilityPolicy: version.resultVisibilityPolicy || 'IMMEDIATE',
        resultDetails: JSON.parse(JSON.stringify(version.resultDetails || DEFAULT_RESULT_DETAILS)),
      },
      answers: [],
      focusLosses: 0,
  ...
  async submitAttempt(
    ...
    if (attempt.status === 'SUBMITTING') {
      ...
      attempt.status = 'SUBMITTED';
      attempt.recoveryStatus = 'SUBMITTED';
    ...
    if (!['ACTIVE', 'PAUSED', 'RECOVERY_PENDING', 'READY', 'DEVICE_LOST'].includes(attempt.status))
    ...
    if (attempt.synchronizationState === 'RECOVERY_PENDING') {
      const submissionEvent: SyncEvent = {
        id: randomId('sync_sub'),
        eventId: randomId('sub_evt'),
        sessionId: attempt.sessionId,
        entity: 'SUBMISSION',
        entityId: attempt.id,
        sourceServerId: 'local-device',
        authorityEpoch:
          this.state.sessions.find((item) => item.id === attempt.sessionId)?.authorityEpoch || 1,
        revision: attempt.localRevision || 1,
      ...
      this.state.syncEvents.push(submissionEvent);
      await this.save();
    }
  ...
  getStudentVisibleResult(attemptId: string, studentId?: string): StudentVisibleResult {
    const attempt = this.requireAttempt(attemptId);
    if (studentId && attempt.studentId !== studentId) {
      throw new Error('Access denied: attempt does not belong to the requested student identity.');
    }
    const version = this.state.versions.find((v) => v.id === attempt.examVersionId);
    const exam = this.state.exams.find((e) => e.id === attempt.examId);
    const examTitle = version?.title || exam?.title || 'Examination';

    // Completed attempts are governed by their immutable settingsSnapshot recorded at creation time
    const policy: ResultVisibilityPolicy =
      attempt.settingsSnapshot?.resultVisibilityPolicy ||
      version?.resultVisibilityPolicy ||
      'IMMEDIATE';
    const detailControls: ExamResultDetailSettings = {
      ...DEFAULT_RESULT_DETAILS,
      ...(attempt.settingsSnapshot?.resultDetails || version?.resultDetails || {}),
    };
    ...
    if (!isSubmitted) {
      return {
        visible: false,
        status: 'HIDDEN',
        message: 'This examination attempt is not yet submitted.',
        attemptId: attempt.id,
    ...
    // 1. NEVER policy: Result remains available to authorized administrators but is NEVER exposed through student result interface
    if (policy === 'NEVER') {
      return {
        visible: false,
        status: 'NEVER',
    ...
    // 2. ON_RELEASE policy: Must be explicitly released by an authorized administrator
    if (policy === 'ON_RELEASE') {
      const releaseRecord = this.state.resultReleases.find((r) => r.attemptId === attempt.id);
      const isReleased = releaseRecord?.status === 'RELEASED';
      ...
      if (!isReleased) {
        return {
          ...
          status: 'HIDDEN',
          message:
            'Your examination has been submitted successfully. Your result will be available when released by the administrator.',
          attemptId: attempt.id,
          examId: attempt.examId,
          examTitle,
          submittedAt: attempt.submittedAt,
    ...
    // 3. IMMEDIATE or RELEASED: Build filtered student view respecting detailControls
    const rawResult = this.getExaminationResult(attempt.id);
    if (!rawResult) {
      return {
        visible: false,
        status: 'HIDDEN',
        message: 'Examination result is being calculated. Please check back shortly.',
        attemptId: attempt.id,
        examId: attempt.examId,
        examTitle,
        submittedAt: attempt.submittedAt,
    ...
    const passMark = attempt.settingsSnapshot?.scoring?.passMark ?? 50;
    const passed = rawResult.percentage >= passMark;

    let questionResults: StudentVisibleResult['questionResults'] | undefined = undefined;
    if (detailControls.showAnswerReview && rawResult.questionResults) {
      const questionMap = new Map(version?.questions.map((q) => [q.id, q]) || []);
      questionResults = rawResult.questionResults.map((qr) => {
        const qSnapshot = questionMap.get(qr.questionId);
        return {
          questionId: qr.questionId,
          questionText: qSnapshot?.questionText,
          answer: qr.answer,
          marksAwarded: detailControls.showScore ? qr.marksAwarded : undefined,
          maxMarks: detailControls.showScore ? qr.maxMarks : undefined,
          isCorrect: detailControls.showCorrectAnswers ? qr.isCorrect : undefined,
          correctOption: detailControls.showCorrectAnswers ? qSnapshot?.correctOption : undefined,
          correctAnswer: detailControls.showCorrectAnswers ? qSnapshot?.correctAnswer : undefined,
          explanation: detailControls.showCorrectAnswers ? qSnapshot?.explanation : undefined,
        };
      });
    }
    ...
    return {
      visible: true,
      status: policy === 'IMMEDIATE' ? 'IMMEDIATE' : 'RELEASED',
      message: 'Examination result is available.',
      attemptId: attempt.id,
      examId: attempt.examId,
      examTitle,
      submittedAt: rawResult.submittedAt,
      score: detailControls.showScore ? rawResult.score : undefined,
      maxMarks: detailControls.showScore ? rawResult.maxMarks : undefined,
      percentage: detailControls.showPercentage ? rawResult.percentage : undefined,
      passed: detailControls.showPassFail ? passed : undefined,
      passMark: detailControls.showPassFail ? passMark : undefined,
      durationSeconds: detailControls.showTimeUsed ? rawResult.durationSeconds : undefined,
      questionResults,
    };
  }

  getResultReleaseRecord(attemptId: string): ExamResultReleaseRecord | undefined {
    return this.state.resultReleases.find((r) => r.attemptId === attemptId);
  }

  getResultReleaseStatus(attemptId: string): ResultReleaseStatus {
    const attempt = this.state.attempts.find((a) => a.id === attemptId);
    if (!attempt) return 'HIDDEN';
    const version = this.state.versions.find((v) => v.id === attempt.examVersionId);
    const policy =
      attempt.settingsSnapshot?.resultVisibilityPolicy ||
      version?.resultVisibilityPolicy ||
      'IMMEDIATE';
    if (policy === 'IMMEDIATE') return 'RELEASED';
    if (policy === 'NEVER') return 'HIDDEN';
    const record = this.state.resultReleases.find((r) => r.attemptId === attemptId);
    return record?.status || 'HIDDEN';
  }

  async releaseResultForAttempt(
    attemptId: string,
    adminId: string,
    adminDeviceSessionId: string,
    reason = 'Administrator released student result',
  ): Promise<ExamResultReleaseRecord> {
    this.requireAdminDevice(adminDeviceSessionId);
    const attempt = this.requireAttempt(attemptId);
    const session = this.state.sessions.find((s) => s.id === attempt.sessionId);
    const epoch = session?.authorityEpoch || 1;

    let record = this.state.resultReleases.find((r) => r.attemptId === attempt.id);
    const previousState: ResultReleaseStatus = record ? record.status : 'HIDDEN';
    ...
    if (!record) {
      record = {
        ...
        releasedByDeviceSessionId: adminDeviceSessionId,
        reason,
        authorityEpoch: epoch,
        updatedAt: now,
      };
      this.state.resultReleases.push(record);
    } else {
      record.status = 'RELEASED';
      record.releasedAt = now;
      record.releasedByAdminId = adminId;
      record.releasedByDeviceSessionId = adminDeviceSessionId;
    ...
    // Record audit trail
    const auditRecord: ResultReleaseAuditRecord = {
      id: randomId('audit_rel'),
      examId: attempt.examId,
      examVersionId: attempt.examVersionId,
      attemptId: attempt.id,
      studentId: attempt.studentId,
    ...
    this.state.resultReleaseAudits.push(auditRecord);
    ...
    await this.logSecurityEvent({
      ...
      sessionId: attempt.sessionId,
      studentId: attempt.studentId,
      deviceSessionId: adminDeviceSessionId,
      type: 'RESULTS_RELEASED',
      severity: 'info',
      details: `Result released by administrator ${adminId}. Reason: ${reason}.`,
    });
  ...
  async revokeResultForAttempt(
    attemptId: string,
    adminId: string,
    adminDeviceSessionId: string,
    reason = 'Administrator revoked student result visibility',
  ): Promise<ExamResultReleaseRecord> {
    this.requireAdminDevice(adminDeviceSessionId);
    const attempt = this.requireAttempt(attemptId);
    const session = this.state.sessions.find((s) => s.id === attempt.sessionId);
    ...
    if (!record) {
      ...
      record.revokedByAdminId = adminId;
      record.reason = reason;
      record.authorityEpoch = epoch;
      record.updatedAt = now;
    }
    ...
    await this.logSecurityEvent({
      ...
      studentId: attempt.studentId,
      deviceSessionId: adminDeviceSessionId,
      type: 'RESULTS_REVOKED',
      severity: 'info',
      details: `Result visibility revoked by administrator ${adminId}. Reason: ${reason}.`,
    });
  ...
  async releaseAllResults(
    ...
    adminId: string,
    adminDeviceSessionId: string,
    reason = 'Administrator released all results for session',
  ): Promise<{ releasedCount: number; records: ExamResultReleaseRecord[] }> {
    this.requireAdminDevice(adminDeviceSessionId);
    const attempts = this.state.attempts.filter(
      (a) =>
        a.sessionId === sessionId &&
        (a.status === 'SUBMITTED' || a.status === 'KIOSK_RELEASED' || a.status === 'CLOSED'),
    );
    const records: ExamResultReleaseRecord[] = [];
    for (const attempt of attempts) {
      const record = await this.releaseResultForAttempt(
        attempt.id,
        adminId,
        adminDeviceSessionId,
        reason,
    ...
    return { releasedCount: records.length, records };
  }

  async releaseSelectedResults(
    sessionId: string,
    attemptIds: string[],
    adminId: string,
    adminDeviceSessionId: string,
    reason = 'Administrator released selected results',
  ): Promise<{ releasedCount: number; records: ExamResultReleaseRecord[] }> {
    this.requireAdminDevice(adminDeviceSessionId);
  ...
  async revokeAllResults(
    sessionId: string,
    adminId: string,
    adminDeviceSessionId: string,
    reason = 'Administrator revoked all results for session',
  ): Promise<{ revokedCount: number; records: ExamResultReleaseRecord[] }> {
    this.requireAdminDevice(adminDeviceSessionId);
    const attempts = this.state.attempts.filter((a) => a.sessionId === sessionId);
    const records: ExamResultReleaseRecord[] = [];
    for (const attempt of attempts) {
      const record = await this.revokeResultForAttempt(
        attempt.id,
        adminId,
        adminDeviceSessionId,
        reason,
    ...
    return { revokedCount: records.length, records };
  }

  async revokeSelectedResults(
    sessionId: string,
    attemptIds: string[],
    adminId: string,
    adminDeviceSessionId: string,
    reason = 'Administrator revoked selected results',
  ): Promise<{ revokedCount: number; records: ExamResultReleaseRecord[] }> {
    this.requireAdminDevice(adminDeviceSessionId);
    const attemptIdSet = new Set(attemptIds);
    const attempts = this.state.attempts.filter(
      (a) => a.sessionId === sessionId && attemptIdSet.has(a.id),
    );
    const records: ExamResultReleaseRecord[] = [];
    for (const attempt of attempts) {
      const record = await this.revokeResultForAttempt(
  ...
  getResultReleaseAudits(sessionIdOrExamId?: string): ResultReleaseAuditRecord[] {
    if (!sessionIdOrExamId) return [...this.state.resultReleaseAudits];
    return this.state.resultReleaseAudits.filter(
      (a) =>
        a.examId === sessionIdOrExamId ||
        this.state.attempts.some(
          (att) => att.id === a.attemptId && att.sessionId === sessionIdOrExamId,
        ),
    );
  }
  ...
  async processIncomingSyncEvents(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!this.syncBatchRunning) {
        void this.drainSyncBatch();
      }
    });
  }

  private async drainSyncBatch(): Promise<void> {
    if (this.syncBatchRunning) return;
    this.syncBatchRunning = true;
    try {
      while (this.pendingSyncItems.length > 0) {
        const batch = this.pendingSyncItems.splice(0);
        let batchTotalApplied = 0;
        ...
        const batchStateBackup = JSON.parse(JSON.stringify(this.state)) as ExaminationState;

        for (const item of batch) {
          const session = this.requireSession(item.sessionId);
          let applied = 0;
          const conflicts: string[] = [];
          const acknowledgedEventIds: string[] = [];

          for (const incoming of item.events) {
            ...
            const syncEventsLen = this.state.syncEvents.length;
            const securityEventsLen = this.state.securityEvents.length;
            const answersBackup = [...this.state.answers];
            const attempt = this.state.attempts.find((a) => a.id === this.incomingAttemptId(incoming));
            const attemptBackup = attempt ? { ...attempt, answers: [...attempt.answers] } : null;
            const sessionBackupRev = session.lastReplicationRevision;

            const reason = this.validateIncomingEvent(session, incoming, item.options);
            if (reason) {
              const rejected = existing || {
                ...incoming,
                status: 'REJECTED' as const,
                serverReceiptAt: item.authoritativeAt,
              };
            ...
            try {
              ...
              this.applyIncomingEventInMemory(session, incoming, serverRevision, item.authoritativeAt);
              const appliedEvent = this.state.syncEvents.find(
                (event) => event.eventId === eventKey || event.id === incoming.id,
              );
              if (appliedEvent) {
                appliedEvent.status = 'APPLIED';
                appliedEvent.serverReceiptAt = item.authoritativeAt;
              } else {
                this.state.syncEvents.push({
                  ...incoming,
                  status: 'APPLIED',
              ...
              if (attempt && attemptBackup) Object.assign(attempt, attemptBackup);
              this.state.answers = answersBackup;
              this.state.syncEvents.length = syncEventsLen;
              this.state.securityEvents.length = securityEventsLen;
              session.lastReplicationRevision = sessionBackupRev;
              conflicts.push(
        ...
        if (batchTotalApplied > 0) {
          const persisted = await this.save();
          if (!persisted) {
            this.state = batchStateBackup;
            for (const { item } of itemResults) {
              item.resolve({
                ok: false,
                applied: 0,
                conflicts: ['Durable examination persistence failed.'],
                revision:
                  batchStateBackup.sessions.find((s) => s.id === item.sessionId)
                    ?.lastReplicationRevision || 0,
                acknowledgedEventIds: [],
              });
            }
  ...
  private validateIncomingEvent(
    ...
    } else if (event.entity === 'SUBMISSION' || event.entity === 'ATTEMPT') {
      // Valid lifecycle synchronization event
  ...
  private applyIncomingEventInMemory(
    ...
    } else if (event.entity === 'SUBMISSION') {
      const payload = event.payload || {};
      const trigger = (payload.trigger as StudentAttempt['submissionTrigger']) || 'MANUAL';
      if (attempt.status !== 'SUBMITTED') {
        attempt.status = 'SUBMITTED';
        attempt.submissionState = trigger === 'EXPIRY' ? 'EXPIRED' : 'SUBMITTED';
        attempt.submittedAt = event.at || authoritativeAt;
        attempt.submissionTrigger = trigger;
        attempt.serverRevision = serverRevision;
        attempt.lastSyncedAt = authoritativeAt;
        attempt.synchronizationState = 'SYNCHRONIZED';
  ...
  async markDeviceDisconnected(
    ...
    for (const attempt of this.state.attempts.filter(
      ...
      attempt.recoveryStatus = 'DEVICE_LOST';
      attempt.synchronizationState = 'RECOVERY_PENDING';
      const now = new Date().toISOString();
      const recovery = {
        id: randomId('recovery'),
  ...
  async markSynchronizationUnavailable(
    ...
    for (const attempt of this.state.attempts.filter(
      (item) => item.sessionId === sessionId && ['ACTIVE', 'PAUSED', 'DEVICE_LOST'].includes(item.status),
    )) {
      attempt.synchronizationState = pendingTooLong ? 'RECOVERY_PENDING' : 'DEGRADED';
      if (pendingTooLong) {
        attempt.status = 'RECOVERY_PENDING';
        attempt.recoveryStatus = 'RECOVERY_PENDING';
        const now = new Date().toISOString();
        const recovery = {
          id: randomId('recovery'),
          sessionId,
