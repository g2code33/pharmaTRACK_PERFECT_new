...
export interface LanExamTransport {
  ...
  fetchStudentResult?(
    ...
    attemptId: string,
    studentId?: string,
  ): Promise<{ ok: boolean; result?: StudentVisibleResult }>;
}
...
export class LanExamClient implements LanExamTransport {
  ...
  fetchStudentResult(sessionId: string, attemptId: string, studentId = this.studentId) {
    const q = studentId ? `?studentId=${encodeURIComponent(studentId)}` : '';
    return this.call<{ ok: boolean; result?: StudentVisibleResult }>(
      `/pharmaexam/v1/sessions/${encodeURIComponent(sessionId)}/attempts/${encodeURIComponent(attemptId)}/student-result${q}`,
    );
  }
...
export class LocalExamAuthority implements LanExamTransport {
  ...
  async fetchStudentResult(sessionId: string, attemptId: string, studentId?: string) {
    const attempt = this.repository.snapshot.attempts.find((item) => item.id === attemptId);
    if (!attempt || attempt.sessionId !== sessionId) {
      throw new Error('Attempt does not belong to this session.');
    }
    const result = this.repository.getStudentVisibleResult(attemptId, studentId);
...
/**
 * In-memory HTTP fetch router for the standard LAN examination protocol.
 * Dispatches HTTP requests directly to the examination authority, ensuring
 * that the Web/PWA client exercises the exact same endpoints, headers,
 * JSON payloads, and error status codes as native PC and Android clients.
 */
export function createLanServerFetch(
  authority: LocalExamAuthority,
  interceptor?: (url: string, init?: RequestInit) => Response | Promise<Response | null> | null,
): typeof fetch {
  return async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    if (interceptor) {
      const intercepted = await interceptor(urlStr, init);
      if (intercepted) return intercepted;
    }
    const url = new URL(urlStr, 'http://192.168.1.100:8787');
    const path = url.pathname;
    const method = init?.method || 'GET';
    const body = init?.body ? JSON.parse(init.body as string) : {};

    try {
      if (path === '/pharmaexam/v1/health' && method === 'GET') {
      ...
      const sessionMatch = path.match(/^\/pharmaexam\/v1\/sessions\/([^/]+)(\/.*)?$/);
      if (sessionMatch) {
        const sessionId = decodeURIComponent(sessionMatch[1]);
        const subpath = sessionMatch[2] || '';

        if (subpath === '/connect' && method === 'POST') {
          const res = await authority.connect(sessionId, body.deviceId, body.role, {
        ...
        if (subpath === '/attempts' && method === 'POST') {
          const res = await authority.createAttempt(sessionId, body.studentId, body.deviceSessionId);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
        ...
        const answerMatch = subpath.match(/^\/attempts\/([^/]+)\/answers$/);
        if (answerMatch && method === 'POST') {
          const attemptId = decodeURIComponent(answerMatch[1]);
          const res = await authority.submitAnswer(sessionId, attemptId, body.answer);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
        ...
        if (subpath === '/sync' && method === 'POST') {
          const res = await authority.sync(sessionId, body.events);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
        ...
        if (subpath === '/heartbeat' && method === 'POST') {
          const res = await authority.heartbeat(sessionId, body.deviceSessionId);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
        ...
        const submitMatch = subpath.match(/^\/attempts\/([^/]+)\/submit$/);
        if (submitMatch && method === 'POST') {
          const attemptId = decodeURIComponent(submitMatch[1]);
          const res = await authority.submitAttempt(sessionId, attemptId, body.deviceSessionId);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
        ...
        const recoverMatch = subpath.match(/^\/attempts\/([^/]+)\/recover$/);
        if (recoverMatch && method === 'POST') {
          const attemptId = decodeURIComponent(recoverMatch[1]);
          const res = await authority.recover(sessionId, attemptId, body.state);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
        ...
        if (subpath === '/package' && method === 'GET') {
          const res = await authority.fetchPackage(sessionId);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
        ...
        const studentResultMatch = subpath.match(/^\/attempts\/([^/]+)\/student-result$/);
        if (studentResultMatch && method === 'GET') {
          const attemptId = decodeURIComponent(studentResultMatch[1]);
          const studentId = url.searchParams.get('studentId') || undefined;
          const res = await authority.fetchStudentResult(sessionId, attemptId, studentId);
          return new Response(JSON.stringify(res), {
            status: 200,
            headers: { 'content-type': 'application/json' },
          });
