  ...
  const generatePackage = async () => {
    ...
    try {
      ...
      const version = await repository.createVersion(exam.id, questions, {
        ...
        resultVisibilityPolicy: draft.resultVisibilityPolicy,
        resultDetails: draft.resultDetails,
      });
      ...
      <div className="grid xl:grid-cols-[1fr_1.2fr] gap-6">
        <section className="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 space-y-5">
          ...
          <div className="border-t pt-4 space-y-2">
            ...
            <div className="border-t pt-4 space-y-3">
              <h3 className="font-black text-slate-800 flex items-center gap-2">
                <Eye className="w-4 h-4 text-emerald-700" /> Result visibility after submission
              </h3>
              <div className="space-y-2">
                <label className="flex items-center gap-2 text-sm font-semibold cursor-pointer">
                  <input
                    ...
                    value="IMMEDIATE"
                    checked={draft.resultVisibilityPolicy === 'IMMEDIATE'}
                    onChange={() => updateDraft({ resultVisibilityPolicy: 'IMMEDIATE' })}
                  />
                ...
                <label className="flex items-center gap-2 text-sm font-semibold cursor-pointer">
                  <input
                    ...
                    name="resultVisibilityPolicy"
                    value="ON_RELEASE"
                    checked={draft.resultVisibilityPolicy === 'ON_RELEASE'}
                    onChange={() => updateDraft({ resultVisibilityPolicy: 'ON_RELEASE' })}
                  />
                  Hide result until administrator releases it
                </label>
                <label className="flex items-center gap-2 text-sm font-semibold cursor-pointer">
                  <input
                    type="radio"
                    name="resultVisibilityPolicy"
                    value="NEVER"
                    checked={draft.resultVisibilityPolicy === 'NEVER'}
                    onChange={() => updateDraft({ resultVisibilityPolicy: 'NEVER' })}
                  />
                  Never show result to students
                </label>
              </div>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {(
                    [
                      ...
                      ['showPassFail', 'Pass/Fail'],
                      ['showCorrectAnswers', 'Correct answers'],
                      ['showAnswerReview', 'Answer review'],
                      ['showTimeUsed', 'Time used'],
                    ] as const
                  ).map(([key, label]) => (
                    <label
                      key={key}
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={Boolean(draft.resultDetails[key])}
                        onChange={(e) =>
                          updateDraft({
                            resultDetails: {
                              ...draft.resultDetails,
