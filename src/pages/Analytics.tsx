...
const Analytics: React.FC = () => {
  ...
  const avgQuizScore = totalQuizzes > 0 ? Math.round(state.quizHistory.reduce((sum, q) => sum + q.scorePercentage, 0) / totalQuizzes) : 0;
  const courseProgressData = state.courses.map((course) => ({ name: course.courseCode, progress: getCourseProgress(course.id), loProgress: getLOProgress(course.id) }));
  ...
  const topicProgressData = state.topics.map((topic) => {
    ...
    return { topicId: topic.id, topicName: topic.topicName, courseCode: course?.courseCode || '', progress, total: slides.length };
  }).filter((t) => t.total > 0);
  const weakTopics = [...topicProgressData].sort((a, b) => a.progress - b.progress).slice(0, 5);
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Link to="/courses" className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md hover:border-blue-400 transition-all group block cursor-pointer">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-blue-100 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform"><BookOpen className="w-6 h-6 text-blue-600" /></div>
            <div><p className="text-2xl font-bold text-gray-800">{totalCourses}</p><p className="text-sm font-bold text-gray-400 group-hover:text-blue-500 transition-colors uppercase tracking-widest">Courses ➔</p></div>
          </div>
        </Link>
        <Link to="/materials" className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md hover:border-green-400 transition-all group block cursor-pointer">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-green-100 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform"><CheckCircle2 className="w-6 h-6 text-green-600" /></div>
            <div><p className="text-2xl font-bold text-gray-800">{completedSlides}/{totalSlides}</p><p className="text-sm font-bold text-gray-400 group-hover:text-green-500 transition-colors uppercase tracking-widest">Slides Done ➔</p></div>
          </div>
        </Link>
        <Link to="/questions" className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md hover:border-purple-400 transition-all group block cursor-pointer">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-purple-100 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform"><FileQuestion className="w-6 h-6 text-purple-600" /></div>
            <div><p className="text-2xl font-bold text-gray-800">{practicedQuestions}/{totalQuestions}</p><p className="text-sm font-bold text-gray-400 group-hover:text-purple-500 transition-colors uppercase tracking-widest">Qs Done ➔</p></div>
          </div>
        </Link>
        <Link to="/quiz" className="bg-white rounded-xl p-4 border border-gray-100 shadow-sm hover:shadow-md hover:border-yellow-400 transition-all group block cursor-pointer">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-yellow-100 rounded-lg flex items-center justify-center group-hover:scale-110 transition-transform"><Award className="w-6 h-6 text-yellow-600" /></div>
            <div><p className="text-2xl font-bold text-gray-800">{avgQuizScore}%</p><p className="text-sm font-bold text-gray-400 group-hover:text-yellow-500 transition-colors uppercase tracking-widest">Avg Quiz ➔</p></div>
          </div>
        </Link>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2"><TrendingUp className="w-5 h-5 text-blue-600" /> Course Progress</h3>
          {courseProgressData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}><BarChart data={courseProgressData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="name" tick={{ fontSize: 12 }} /><YAxis domain={[0, 100]} tick={{ fontSize: 12 }} /><Tooltip /><Legend /><Bar dataKey="progress" name="Slide Progress" fill="#2D6A4F" radius={[4, 4, 0, 0]} /><Bar dataKey="loProgress" name="LO Mastery" fill="#FFB703" radius={[4, 4, 0, 0]} /></BarChart></ResponsiveContainer>
          ) : <div className="h-[250px] flex items-center justify-center text-gray-400">No course data yet</div>}
        </div>
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2"><Brain className="w-5 h-5 text-purple-600" /> Quiz Performance Trend</h3>
          {quizPerformanceData.length > 0 ? (
            <ResponsiveContainer width="100%" height={250}><LineChart data={quizPerformanceData}><CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" /><XAxis dataKey="date" tick={{ fontSize: 12 }} /><YAxis domain={[0, 100]} tick={{ fontSize: 12 }} /><Tooltip /><Line type="monotone" dataKey="score" stroke="#8B5CF6" strokeWidth={3} dot={{ r: 4, fill: '#8B5CF6' }} activeDot={{ r: 6 }} /></LineChart></ResponsiveContainer>
          ) : <div className="h-[250px] flex items-center justify-center text-gray-400">No quizzes taken yet</div>}
        </div>
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
      ...
      <div className="grid lg:grid-cols-2 gap-6">
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2"><Award className="w-5 h-5 text-yellow-600" /> Exam Readiness by Course</h3>
          {state.courses.length > 0 ? (
            <div className="space-y-3">
              {state.courses.map((course) => {
                const avgProgress = Math.round((getCourseProgress(course.id) + getLOProgress(course.id)) / 2);
                const status = getReadinessStatus(avgProgress);
                return (
                  <Link key={course.id} to={`/course/${course.id}`} className="block p-3 bg-gray-50 rounded-lg border border-transparent hover:border-blue-300 hover:bg-blue-50 transition-all cursor-pointer">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2"><span className="font-bold text-gray-800">{course.courseCode}</span><span className={`px-2 py-0.5 text-[10px] font-black uppercase tracking-widest rounded ${status.bg} ${status.color}`}>{status.emoji} {status.label}</span></div>
                      <span className="font-black text-gray-800">{avgProgress}%</span>
                    </div>
                    <div className="h-2 bg-gray-200 rounded-full overflow-hidden"><div className={`h-full transition-all ${avgProgress >= 70 ? 'bg-green-500' : avgProgress >= 40 ? 'bg-yellow-500' : 'bg-red-500'}`} style={{ width: `${avgProgress}%` }} /></div>
                  </Link>
                );
              })}
            </div>
        ...
        <div className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm">
          <h3 className="font-semibold text-gray-800 mb-4 flex items-center gap-2"><AlertTriangle className="w-5 h-5 text-red-500" /> Topics Needing Attention</h3>
          {weakTopics.length > 0 ? (
            <div className="space-y-3">
              {weakTopics.map((topic) => (
                <Link key={topic.topicName} to={`/read/${topic.topicId}`} className="flex items-center justify-between p-3 bg-red-50 rounded-lg border border-transparent hover:border-red-300 hover:shadow-sm transition-all cursor-pointer block">
                  <div><p className="font-bold text-gray-800">{topic.topicName}</p><p className="text-xs font-bold text-gray-500 uppercase tracking-widest">{topic.courseCode}</p></div>
                  <span className="font-black text-red-600">{topic.progress}% Mastered</span>
                </Link>
              ))}
            </div>
          ) : topicProgressData.length > 0 ? <div className="text-center py-8"><CheckCircle2 className="w-12 h-12 text-green-400 mx-auto mb-2" /><p className="text-green-600 font-bold">Great job! All topics are on track.</p></div> : <div className="text-center py-8 text-gray-400">No topic data yet</div>}
        </div>
      </div>

      <Link to="/objectives" className="bg-white rounded-xl p-5 border border-gray-100 shadow-sm hover:shadow-md hover:border-[#FFB703] transition-all block cursor-pointer group">
        <h3 className="font-semibold text-gray-800 mb-4 flex items-center justify-between gap-2">
        ...
        {totalLOs > 0 && <div className="mt-4"><div className="flex justify-between text-sm mb-1"><span className="text-gray-600 font-bold">Overall Mastery</span><span className="font-black text-gray-800">{Math.round((masteredLOs / totalLOs) * 100)}%</span></div><div className="h-3 bg-gray-100 rounded-full overflow-hidden"><div className="h-full bg-[#FFB703]" style={{ width: `${(masteredLOs / totalLOs) * 100}%` }} /></div></div>}
      </Link>
    </div>
  );
};
