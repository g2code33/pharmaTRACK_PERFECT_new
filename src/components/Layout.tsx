...
import {
  ...
  LogOut, ChevronLeft, ChevronRight, ShieldCheck, Zap, Bookmark,
  WifiOff, RefreshCw, Download, CheckCircle, Loader2
} from 'lucide-react';
...
const Layout: React.FC = () => {
  ...
  // Extremely robust Global Search
  useEffect(() => {
    ...
    if (query.length > 2) {
      ...
      state.courses.forEach(c => {
        if (c.courseName.toLowerCase().includes(query) || c.courseCode.toLowerCase().includes(query)) {
          results.push({ id: `c-${c.id}`, title: `${c.courseCode}: ${c.courseName}`, subtitle: 'Course', link: `/course/${c.id}` });
        }
      });
      // 2. Search Topics
      state.topics.forEach(t => {
        if (t.topicName.toLowerCase().includes(query)) {
          results.push({ id: `t-${t.id}`, title: t.topicName, subtitle: 'Topic', link: `/read/${t.id}` });
        }
      });
      // 3. Search Slides/Materials
      state.slides.forEach(s => {
        if (s.title.toLowerCase().includes(query) || s.contentText?.toLowerCase().includes(query)) {
          results.push({ id: `s-${s.id}`, title: s.title, subtitle: 'Study Material', link: `/read/${s.topicId}?slide=${s.slideNumber - 1}` });
        }
      });
      // 4. Search Notes
      state.notes.forEach(n => {
        if (n.noteText.toLowerCase().includes(query) || n.noteText.toLowerCase().includes(query)) {
          results.push({ id: `n-${n.id}`, title: n.noteText.substring(0, 30) + "...", subtitle: 'My Note', link: '/notes' });
        }
      });
      // 5. Search Questions
      state.examQuestions.forEach(q => {
        if (q.questionText.toLowerCase().includes(query)) {
          results.push({ id: `q-${q.id}`, title: q.questionText, subtitle: 'Question Bank', link: '/questions' });
        }
      });

      setSearchResults(results.slice(0, 15)); // Limit to top 15 results to prevent lag
    } else {
      setSearchResults([]);
    }
  }, [searchQuery, state]);
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-slate-50 flex-col">
      {isOffline && (
        <div className="w-full bg-red-600 text-white text-xs font-bold text-center py-1.5 uppercase tracking-widest animate-pulse z-[100] relative shadow-md flex items-center justify-center gap-2">
          <WifiOff className="w-4 h-4" /> No Internet Connection - Operating in Offline Mode
        </div>
      )}
      <div className="flex flex-1 overflow-hidden">
        <aside className={`fixed inset-y-0 left-0 z-50 bg-[#0F172A] text-white flex flex-col transition-all duration-300 ease-in-out lg:relative shadow-2xl ${mobileMenuOpen ? 'translate-x-0 w-72' : '-translate-x-full lg:translate-x-0'} ${sidebarCollapsed ? 'lg:w-20' : 'lg:w-72'}`}>
          <div className={`flex items-center p-6 border-b border-white/5 ${sidebarCollapsed ? 'justify-center' : 'gap-3'}`}>
            <div className="w-10 h-10 flex-shrink-0 overflow-hidden rounded-lg shadow-lg shadow-green-500/20"><img src="/logo.png" alt="PharmaTRACK Logo" className="w-full h-full object-cover scale-110" onError={(e) => e.currentTarget.style.display = 'none'} /></div>
            ...
            <div className="flex items-center gap-6">
              ...
              <div className="flex-1 max-w-3xl relative">
                <div className="relative group">
                  ...
                  <input type="text" placeholder="Global Search: Courses, Topics, Slides, Notes, Questions..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-11 pr-12 py-3 bg-gray-100 border-none rounded-2xl text-sm font-medium focus:bg-white focus:ring-4 focus:ring-[#2D6A4F]/10 outline-none transition-all shadow-inner" />
                ...
                {searchResults.length > 0 && (
                  <div className="absolute top-full left-0 w-full mt-2 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-50 max-h-64 overflow-y-auto">
                    {searchResults.map(res => (
                      <Link key={res.id} to={res.link} onClick={() => setSearchQuery('')} className="block px-4 py-3 hover:bg-gray-50 border-b last:border-0 transition-colors">
              ...
              <div className="hidden sm:flex items-center gap-4">
                <div className="flex items-center gap-2 px-3 py-1.5 bg-green-50 rounded-full border border-green-100">
                  ...
                  <span className="text-[10px] font-black text-green-700 uppercase tracking-widest">Cloud Ready</span>
                </div>

                <button onClick={checkForUpdates} disabled={updateStatus === 'checking' || updateStatus === 'downloading'} title="Check for Updates" className="flex items-center gap-2 px-4 py-2 bg-blue-600 text-white hover:bg-blue-700 rounded-full font-bold text-xs transition-all shadow-md disabled:opacity-50">
