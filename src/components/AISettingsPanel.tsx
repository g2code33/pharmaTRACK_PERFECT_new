...
export const AISettingsPanel: React.FC = () => {
  ...
  const [editing, setEditing] = useState<Record<string, Partial<ProviderConfig> & { apiKey?: string }>>({});
  ...
  const runTest = async (provider: ProviderRow) => {
    ...
    try {
      ...
      if (patch.apiKey || patch.model || patch.baseUrl) {
        await ai.saveProvider({ ...provider, ...patch, apiKey: patch.apiKey } as ProviderConfig & { apiKey?: string });
      }
      const result = await ai.testProvider(provider.id, (patch.model ?? provider.model) || undefined);
      setNotice(result.ok ? `${provider.label} connected successfully.` : `${provider.label} failed — see the checks below.`);
    } catch (err) {
  ...
  <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
    ...
    <div className="p-5 space-y-3 text-sm text-gray-600">
      <div className="flex items-start gap-2">
        ...
        <p>
          <strong>Keys are never exported.</strong> API keys are stored separately from your academic data, so semester
          archives and <code>.pharmatrack</code> backups cannot contain them.
        </p>
      </div>
      <div className="flex items-start gap-2">
        <CheckCircle2 className="w-4 h-4 text-green-600 mt-0.5 shrink-0" />
        <p>
          <strong>Only the selected context is sent.</strong> PharmaTRACK sends the slide/page you are looking at, plus
          your question — not your whole semester. You can see exactly what will be sent before each request.
        </p>
      </div>
      <div className="flex items-start gap-2">
      ...
      <div className="flex items-start gap-2">
        <CheckCircle2 className="w-4 h-4 text-green-600 mt-0.5 shrink-0" />
        <p>
          <strong>Keys never appear in URLs.</strong> Providers that support an authentication header are always called
...
const ProvidersCard: React.FC<ProvidersCardProps> = ({
  providers, onAdd, expanded, setExpanded, crud, setField, showKey, setShowKey,
              ...
              {isOpen && (
                <div className="px-4 pb-4 space-y-3 bg-gray-50/60">
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <label className="block">
                      <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Model</span>
                      <input
                    ...
                    {(preset.configurableBaseUrl || provider.kind === 'custom') && (
                      <label className="block">
                        <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Base URL</span>
                        <input
                  ...
                  <div className="grid sm:grid-cols-2 gap-3">
                    <label className="block">
                      ...
                      <input
                        ...
                        onChange={(e) => setField(provider.id, { timeoutMs: Number(e.target.value) * 1000 })}
                        className={`${inputCls} mt-1`}
                      />
                    </label>
                  ...
                  <CapabilityChips provider={provider} patch={patch} onDeclare={(caps) => setField(provider.id, { declaredCapabilities: caps })} />

                  <div className="flex flex-wrap gap-2 pt-1">
                    <button className={btnPrimary} onClick={() => onSave(provider)}>
                      <Save className="w-4 h-4" /> Save
                    </button>
                    <button className={btnGhost} onClick={() => onTest(provider)} disabled={testing === provider.id}>
                      {testing === provider.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />}
                      Test connection
                    </button>
                    {preset.supportsModelList && (
                      <button className={btnGhost} onClick={() => onFetchModels(provider)} disabled={fetching === provider.id}>
                        {fetching === provider.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <RefreshCw className="w-4 h-4" />}
                        Fetch models
                      </button>
                    )}
                    <button className={btnGhost} onClick={() => onRemove(provider.id)} title="Clear key & disable">
...
const StatusLabel: React.FC<{ status: string }> = ({ status }) => {
  ...
  return (
    <span className={`text-[10px] font-black uppercase tracking-wider px-2 py-0.5 rounded-full border ${item.cls}`}>
      {item.text}
    </span>
  );
};
...
const TestReport: React.FC<{ test: AIConnectionTest }> = ({ test }) => (
  ...
  >
    <div className="flex items-center gap-2 mb-1">
      {test.ok ? <CheckCircle2 className="w-4 h-4 text-green-600" /> : <CircleSlash className="w-4 h-4 text-red-600" />}
    ...
    <div>
      <p className="text-[11px] font-bold uppercase tracking-wider text-gray-500 mb-1">
        ...
        <span className="font-normal normal-case text-gray-400">
          {info.unknown ? '(model unknown — only the protocol baseline is claimed)' : `(from ${info.source})`}
        </span>
      </p>
      ...
      {info.contextWindow ? (
        ...
        <p className="text-[10px] text-gray-400 mt-1">Context size not established for this model.</p>
      )}
    </div>
  );
};
...
const ProfileRow: React.FC<{
  ...
  const providerLabel = (id: ProviderId) => providers.find((p) => p.id === id)?.label ?? (id || 'not set');
    ...
    <div>
        ...
        <button className="flex items-center gap-3 min-w-0 text-left" onClick={onToggle}>
          ...
          <div className="min-w-0">
            <p className="font-bold text-sm text-gray-800 truncate">
              {profile.name} {active && <span className="text-[10px] font-black uppercase text-[#2D6A4F]">active</span>}
            </p>
            <p className="text-[11px] text-gray-500 truncate">
              {providerLabel(profile.providerId)} · {profile.model || 'provider default'}
              {profile.fallbacks.length ? ` · fallback: ${profile.fallbacks.map(providerLabel).join(' → ')}` : ''}
            </p>
          </div>
        </button>
        <div className="flex items-center gap-2 shrink-0">
          ...
          {profile.id !== 'default' && (
            <button className={btnGhost} onClick={() => ai.removeProfile(profile.id)} title="Delete profile">
      ...
      {open && (
        <div className="px-4 pb-4 space-y-3 bg-gray-50/60">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            <label className="block">
              <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Provider</span>
              <select
            ...
            <label className="block">
              <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Model override</span>
              <input
                value={draft.model ?? ''}
          ...
          <div className="grid sm:grid-cols-3 gap-3">
            <label className="block">
              <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Temperature</span>
              <input
                type="number"
                step="0.1"
                min={0}
            ...
            <label className="block">
              <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Max output tokens</span>
              <input
                type="number"
                min={64}
                max={32_000}
            ...
            <label className="block">
              <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Context budget (tokens)</span>
              <input
                type="number"
                min={500}
                max={200_000}
          ...
          <label className="block">
            <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">System instructions</span>
            <textarea
          ...
          <div>
            <span className="text-[11px] font-bold uppercase tracking-wider text-gray-500">Fallback providers (in order)</span>
                ...
                return (
                  <button
                    ...
                    onClick={() =>
                      setDraft({
                        ...draft,
                        fallbacks: on ? draft.fallbacks.filter((f) => f !== p.id) : [...draft.fallbacks, p.id],
                      })
                    }
                    className={`px-2 py-1 rounded-lg text-[11px] font-bold border disabled:opacity-40 ${
                      on ? 'bg-[#2D6A4F] text-white border-[#2D6A4F]' : 'bg-white text-gray-600 border-gray-200'
                    }`}
                  >
                    {p.label}
                  </button>
    ...
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="p-5 space-y-3">
        <label className="flex items-center gap-2 text-sm text-gray-700">
          ...
          />
          Automatic fallback — if the selected provider fails, try the next one (you are always told which provider answered)
        </label>
        ...
        <div>
          ...
          <ol className="space-y-1">
            {ordered.map((provider, index) => (
              <li key={provider.id} className="flex items-center justify-between gap-2 p-2 rounded-lg border border-gray-100">
                ...
                <span className="flex gap-1">
                  ...
                  <button className="p-1 text-gray-400 hover:text-gray-700" onClick={() => move(index, 1)} title="Move down">
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="p-5 text-sm text-gray-600 space-y-3">
        <p>
          Removes every stored API key and returns all AI settings to their defaults. Your courses, materials, notes and
          past conversations are not touched.
        </p>
        {confirming ? (
          <div className="flex gap-2">
            <button
